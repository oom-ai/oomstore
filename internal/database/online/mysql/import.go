...
func (db *DB) Import(ctx context.Context, opt online.ImportOpt) error {
	...
	err := dbutil.WithTransaction(db.DB, ctx, func(ctx context.Context, tx *sqlx.Tx) error {
		...
		schema, err := dbutil.BuildFeatureDataTableSchema(tableName, opt.Entity, opt.FeatureList, types.MYSQL)
		if err != nil {
			return err
		}
		...
		for record := range opt.ExportStream {
			...
			if len(records) == PostgresBatchSize {
				if err := dbutil.InsertRecordsToTableTx(tx, ctx, tableName, records, columns, types.MYSQL); err != nil {
					return err
				}
				records = make([]interface{}, 0, PostgresBatchSize)
			}
